...
var findExp = { 'excuteData.attach': { $exists: true, $ne: [] } };

// cc_data  account business attach exporter
var businessTask = {
        ...
        mongo.db('bill').collection('platform_account_product').find({ _id: { $regex: /.+_cc/ } }).toArray(function (err, docs) { // 查找有cc product
            ...
            Thenjs()
                .eachSeries(docs, function (cont, data) {
                    ...
                    cont(null, { dbName: data.dataDB, collectionName: info[1] + '_business_history' });
                })
    ...
    query: function (data, callback) {
        ...
        mongo.db(dbName).collection(collectionName).find(findExp).skip(skip).limit(limit).toArray(function (err, docs) {
            if (!err) {
                try {
                    var res = [];
                    var account = /(.+)_business_history/.exec(collectionName)[1];
                    docs.forEach(function (data) {
                        var bucketName = 'm7-test';
                        var dataPaths = [];
                        try {
                            if (data.excuteData.attach instanceof Array) {
                                data.excuteData.attach.forEach(function (value) {
                                    if (value.id && value.id.indexOf(account + '/business') === -1) return; // 过滤不是bucket的数据
                                    dataPaths.push(value.id);
                                });
